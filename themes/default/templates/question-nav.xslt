...
<nav class="question-nav navbar" data-bind="jmescope: question.scope,attr: {{'aria-label': R('question.nav.label')}}">
    ...
    <button type="button" class="btn default nextQuestionBtn" data-bind="visible: $root.exam().mode()=='normal' &amp;&amp; $root.exam().exam.settings.navigateMode=='diagnostic', click: Numbas.controls.nextQuestion, attr: {{disabled: !$parent.canAdvance()}}"><localise>control.move to next question</localise></button>
    <button class="btn default regenBtn" data-bind="visible: $root.exam().mode()=='normal' &amp;&amp; $root.exam().exam.settings.allowRegen, click: Numbas.controls.regenQuestion"><localise>control.regen</localise></button>
    <button class="btn default revealBtn" data-bind="visible: question.parts.length &amp;&amp; canReveal, click: Numbas.controls.revealAnswer"><localise>control.reveal</localise></button>
</nav>
{% endraw %}
