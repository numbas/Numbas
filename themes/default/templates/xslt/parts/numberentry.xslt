...
<xsl:template match="part[@type='numberentry']" mode="typespecific">
    ...
    <input type="text" autocapitalize="off" inputmode="text" spellcheck="false" step="{answer/inputstep/@value}" class="numberentry" data-bind="event: inputEvents, textInput: studentAnswer, autosize: true, disable: disabled, css: {{'has-error': warningsShown}}, attr: {{title: input_title}}, part_aria_validity: hasWarnings, part: $data"/>
