...
<xsl:template match="part">
	...
	<xsl:if test="parent::parts">
		<xsl:if test="count(../part[not(@type='information')]) &gt; 1">
			<h4 class="partheader"><xsl:number count="part[not(@type='information')]" format="a) "/></h4>
		</xsl:if>
	</xsl:if>
	<xsl:element name="{$tag}">
		...
		<xsl:if test="not(ancestor::gaps)">
			<div class="container-fluid">
				<div class="row">
					<div class="partFeedback col-xs-12 col-sm-3 col-md-3 col-lg-2 well pull-right" data-bind="visible: showFeedbackBox">
						<button class="btn btn-primary btn-block submitPart" data-bind="css: {{dirty: isDirty}}, click: controls.submit, disable: !showSubmitPart()"><localise>question.submit part</localise></button>
				...
				<div class="row">
					<ol class="feedbackMessages well col-lg-6 col-md-6 col-xs-12 pull-right" data-bind="slideVisible: feedbackMessages().length, foreach: feedbackMessages">
